    ...
    try {
      ...
      for (let i = 0; i < guests.length; i++) {
        ...
        try {
          ...
          errors.push(`Failed to add guest ${guest.firstname} ${guest.lastname}: ${error}`)
        }
      ...
      toast.success(`Successfully imported ${importedGuests.length} out of ${guests.length} guests`)
      onSuccess()
    } catch (error) {
        ...
        <div className="grid gap-4 py-4">
          ...
          {isLoading && importProgress && (
            <div className="text-center">
              ...
              <Progress value={(importProgress.current / importProgress.total) * 100} className="mt-2" />
              <p className="text-sm mt-1">
                Processed: {importProgress.current} / {importProgress.total}
                {' '}(Success: {importProgress.success}, Failure: {importProgress.failure})
              </p>
            </div>
          )}
          {error && (
